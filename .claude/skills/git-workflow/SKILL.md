---
name: git-workflow
description: "커밋·이슈·PR 워크플로우를 표준화하는 스킬. commit(Conventional Commits 강제, AI 흔적 제거 — Co-Authored-By 트레일러 금지), issue([feat]/[bug]/[chore] 템플릿 생성, 라벨 자동 태깅), PR(이슈 번호 포함 제목, 이모지·코드 블럭 없는 본문 양식, Generated with Claude Code 푸터 금지, CodeRabbit 트리거 확인) 세 가지 워크플로우 제공. 커밋 메시지·이슈·PR 본문의 자유 서술 부분은 생성 직전 Humanize KR(humanize-korean@im-not-ai)로 다듬어 'AI가 쓴 티'를 제거. '커밋해줘', '이슈 만들어줘', 'PR 올려줘', '커밋하고 PR 생성해줘' 요청 시 반드시 이 스킬을 사용. springboot-dev 구현 완료 후에는 사용자 승인 시에만 실행."
---
...
| PR: `작업 내용`·`특이사항` 서술 | 섹션 헤더·표 틀·`Closes #<번호>` |
...
## 공통 2: PR·커밋 본문 스타일 (이모지·코드 블럭 금지)

AI가 쓴 커밋·PR은 큰 코드 블럭이나 diff 덤프를 본문에 통째로 붙이고 이모지로 섹션을
꾸미는 경향이 있다. 사람은 그러지 않는다 — 변경은 이미 PR "Files changed"에 있으니
본문에서 되풀이하지 않고, 섹션 헤더도 이모지 없이 평범한 텍스트로 쓴다.

- **PR 본문에는 fenced 코드 블럭(```)을 넣지 않는다 — 크기·언어 무관, 예외 없이 완전 금지.**
  GitHub에서 글자 뒤 배경 박스로 렌더링돼 기계가 쓴 티가 난다. 커밋·이슈 본문도 코드 블럭 없이 쓴다.
- **PR 제목·본문에 이모지를 쓰지 않는다** — 섹션 헤더도 `## 작업 내용`처럼 이모지 없이 단다.
- 변경 지점은 **인라인 백틱**으로만 가리킨다 — `파일:라인`, `메서드명`, 경로, 식별자.
  (인라인 백틱은 허용 — 금지 대상은 여러 줄 코드 블럭과 이모지뿐이다.)
- 코드를 꼭 보여야 하면 그 몇 줄을 산문으로 풀거나 인라인 백틱으로 쓰고, 정 필요하면 PR
  코멘트(본문 아님)에 남긴다.
...
> 먼저 **공통 0(AI 흔적 금지)** 로 생성 도구 푸터·서명을 넣지 않고, **공통 2**로 이모지·코드
> 블럭을 배제한 뒤, **공통 1(humanize)** 로 `작업 내용`·`특이사항` 서술을 다듬어 아래를 실행한다.

**본문 양식** — 팀 리뷰 노트처럼 자연스러운 서술을 기본으로, 필요한 곳에만 표/불릿을 쓴다.
섹션 헤더는 이모지 없이 달고, 본문에 코드 블럭(```)은 넣지 않는다(공통 2).
...
### 1. <작업 단위 제목>
<맥락·결정 이유를 풀어 쓴 서술. 필요하면 핵심 경로·식별자를 인라인 백틱으로 인용>
...
- 섹션 헤더·본문에 이모지를 붙이지 않는다(공통 2) — `## 작업 내용`·`## 테스트`처럼 평문 헤더로만.
- 고정 체크리스트를 기계적으로 나열하지 않는다. 검증한 백엔드 관심사(N+1·RBAC·트랜잭션·멱등성·환경변수)는 `테스트`/`특이사항`에 **실제로 한 것만** 서술한다.
- 본문에 fenced 코드 블럭(```)을 넣지 않는다(공통 2, 완전 금지). 변경 지점은 인라인 `파일:라인`·경로·식별자로 가리키고, 코드는 산문으로 풀어 쓴다.
