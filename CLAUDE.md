...
| 2026-08-13 | **OCR 아웃박스 클래스 `Kafka*` → `Ocr*` 리네임(테이블은 보류)** — 브로커가 SQS로 바뀌었는데 클래스에 옛 브로커 이름이 남아 stale하던 것을 정리. `KafkaOutboxEvent`/`KafkaOutboxStatus`/`KafkaOutboxRepository`→`OcrOutbox*`, 참조 4곳(`OutboxRelay`·`OcrJobOutboxPort(Impl)`·`OutboxRelayTest`)·stale 주석("Kafka로 발행"·"Kafka 파티션 키") 동반 정리. **테이블 `kafka_outbox_events` 리네임은 되돌렸다** — 처음엔 V40으로 `ocr_outbox_events` 리네임을 넣었으나, PostgreSQL `ALTER TABLE/INDEX ... RENAME TO`가 대상 스키마 `CREATE` 권한을 요구해(실측 확인) `public`에 권한 없는 운영 유저에서 #223과 동일하게 배포 실패함을 발견해 마이그레이션을 제거했다. 이 교훈을 Key Configuration의 스키마 규칙에 권한 매트릭스로 반영(이름 해석≠권한, `RENAME`·`ADD CONSTRAINT UNIQUE`도 `CREATE` 권한 필요). CLAUDE.md의 두 아웃박스 혼동 서술도 정정(SQS 릴레이는 `OutboxProcessor`가 아니라 `OutboxRelay`, `outbox_events`=회원 탈퇴 후처리 / `kafka_outbox_events`=OCR 발행). | CLAUDE.md, infra/outbox/OcrOutbox*, infra/sqs/* | 브로커명이 박혀 stale해진 이름 정리 요청 — 클래스만 정리하고 테이블은 권한 정리 후 별도 처리 |
