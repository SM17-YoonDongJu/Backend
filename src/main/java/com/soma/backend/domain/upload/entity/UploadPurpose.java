...
/**
 * 업로드 용도 값 객체(VO). 각 용도가 소유하는 S3 key prefix와 허용 MIME 화이트리스트를 캡슐화하고,
 * 허용되지 않은 content_type을 거부하는 불변식을 리치 도메인으로 표현한다. DB·JSON 모두 소문자
 * 문자열 값을 사용한다(report 도메인 AccidentType 선례와 동일한 @JsonValue/@JsonCreator 관례).
 */
public enum UploadPurpose {
  REPORT_DOCUMENT("report_document", "report-documents", Types.DOCUMENT),
  LICENSE("license", "licenses", Types.DOCUMENT),
  REGISTRATION("registration", "registrations", Types.DOCUMENT),
  AVATAR("avatar", "avatars", Types.IMAGE);

  private final String value;
  private final String keyPrefix;
  private final Set<String> allowedContentTypes;

  UploadPurpose(String value, String keyPrefix, Set<String> allowedContentTypes) {
    this.value = value;
