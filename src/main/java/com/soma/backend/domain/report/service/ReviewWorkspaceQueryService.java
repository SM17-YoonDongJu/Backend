...
public class ReviewWorkspaceQueryService {
  ...
  public ReviewWorkspaceResponse getReviewWorkspace(UUID reportId, UUID adjusterId) {
    ...
    // additional_information·description은 암호화(bytea) 컬럼이라 native 프로젝션이 아니라 컨버터가 적용되는
    ...
    String description = claim == null ? null : claim.getDescription();
    ...
    return ReviewWorkspaceResponse.from(
        report, context, region, claimDetails, additionalInformation, description, aiIssues, review, attachments,
