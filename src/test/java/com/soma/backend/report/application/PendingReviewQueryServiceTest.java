...
class PendingReviewQueryServiceTest {
  ...
  void summaryUsesSlaThreshold() {
    given(reportQueryRepository.countPending()).willReturn(9L);
    given(reportQueryRepository.countDueSoon(any(LocalDateTime.class))).willReturn(3L);

    LocalDateTime before = LocalDateTime.now().minusDays(5);
    PendingReviewSummaryResult summary = service.getSummary();
    ...
    verify(reportQueryRepository).countDueSoon(captor.capture());
  ...
  void listMapsDerivedFields() {
    ...
    given(reportQueryRepository.findPendingReviewRows(null, null, null, reportId, pageable))
        .willReturn(page);

    Page<PendingReviewItemResult> result =
        service.getPendingReviewList(null, null, null, reportId, pageable);

    PendingReviewItemResult item = result.getContent().get(0);
  ...
  void listNullSafe() {
    ...
    Pageable pageable = PageRequest.of(0, 20);
    given(reportQueryRepository.findPendingReviewRows(null, null, null, adjusterId, pageable))
        .willReturn(new PageImpl<>(List.of(row), pageable, 1));

    PendingReviewItemResult item =
        service.getPendingReviewList(null, null, null, adjusterId, pageable).getContent().get(0);
