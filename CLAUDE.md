...
**infra/outbox** — 트랜잭셔널 아웃박스 패턴. 도메인 트랜잭션과 같은 커밋으로 이벤트를 적재하고, 별도 스케줄러가 `FOR UPDATE SKIP LOCKED`로 폴링해 처리한다 (부수효과의 원자성·재시도 보장, 다중 인스턴스 중복 처리 방지). 용도가 다른 **두 아웃박스가 공존**한다 — `outbox_events`(`OutboxEvent`)는 `OutboxProcessor`가 회원 탈퇴 후처리(Redis 토큰 정리·Apple 토큰 폐기)를 수행하고, `ocr_outbox_events`(`OcrOutboxEvent`)는 `OutboxRelay`가 OCR 트리거를 SQS로 발행한다. 발행 대상 큐는 아웃박스 `topic` 컬럼(=SQS 큐 이름, `app.sqs.ocr-queue-name`)이며 브로커는 관리형 AWS SQS다. OCR 아웃박스 테이블은 `kafka_outbox_events`로 생성됐다가(V13) 브로커 전환 후 `ocr_outbox_events`로 리네임했다(V40) — 브로커명 대신 용도로 명명한다.
...
| 2026-08-13 | **OCR 아웃박스 `kafka_` → `ocr_` 리네임** — 브로커가 SQS로 바뀌었는데 스키마·클래스에 옛 브로커 이름이 남아 stale하던 것을 용도 기반 이름으로 교정. 테이블 `kafka_outbox_events`→`ocr_outbox_events`, 인덱스·PK 제약까지 리네임(V40, 순수 `ALTER ... RENAME`이라 데이터·컬럼 보존). 클래스 `KafkaOutboxEvent`/`KafkaOutboxStatus`/`KafkaOutboxRepository`→`OcrOutbox*`, 참조 4곳(`OutboxRelay`·`OcrJobOutboxPort(Impl)`·`OutboxRelayTest`)·stale 주석("Kafka로 발행"·"Kafka 파티션 키") 동반 정리. 테이블 RENAME이 암묵 PK 제약명(`kafka_outbox_events_pkey`)은 안 바꾼다는 점을 로컬 검증으로 발견해 함께 처리. CLAUDE.md의 두 아웃박스 혼동 서술도 정정(SQS 릴레이는 `OutboxProcessor`가 아니라 `OutboxRelay`, `outbox_events`=회원 탈퇴 후처리 / `ocr_outbox_events`=OCR 발행). ⚠️ `@Table` 변경과 V40은 반드시 같은 배포(ddl-auto=validate). | CLAUDE.md, infra/outbox/OcrOutbox*, infra/sqs/*, db/migration/V40 | 브로커명을 스키마에 박아 생긴 stale 이름 정리 요청 — 재발 방지로 브로커명 대신 용도(ocr)로 명명 |
