...
/**
 * {@link OcrJobOutboxPort} 구현체. OcrJob을 JSON 문자열로 직렬화해 OcrOutboxEvent로 저장한다.
...
public class OcrJobOutboxPortImpl implements OcrJobOutboxPort {
  ...
  public void enqueue(OcrJob job) {
    ...
    outboxRepository.save(OcrOutboxEvent.pending(AGGREGATE_TYPE, aggregateId, ocrQueueName, job.jobId(), payload));
  }
}
