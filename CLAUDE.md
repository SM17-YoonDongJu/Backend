...
# QA E2E (Playwright, Gradle과 무관한 별도 npm 프로젝트)
cd qa-e2e && npm install && npx playwright install
npm run test:unauth   # 1단계 — 로그인 불필요
npm run test:auth     # 2단계 — dev-login 세션 필요(app.dev-login.enabled=true)
```
...
│   ├── controller/            # REST 컨트롤러 — ApiResponse<T> 직접 반환, 얇게 유지
...
**global/security** — `JwtProvider`로 토큰 생성·검증, `JwtFilter`(OncePerRequestFilter)로 요청마다 인증 처리, `CustomUserDetails`에 `userId`와 `role`을 담아 `SecurityContext`에 저장한다. 인증 전송은 **HttpOnly 쿠키 단일 경로**다 — `JwtFilter.resolveToken()`은 `access_token` 쿠키에서만 토큰을 읽고 **`Authorization: Bearer` 헤더 인증은 지원하지 않는다**(PR #169에서 제거). 헤더로 우회할 수 없으므로 서버-투-서버 호출·부하 테스트도 쿠키를 실어야 한다. `CookieProvider`가 `access_token`/`refresh_token` 쿠키를 생성·만료·조회하며(`access_token`은 Path `/`, `refresh_token`은 `/auth`로 좁혀 재발급·로그아웃 요청에만 전송) `AuthTokenService`가 발급을 오케스트레이션한다. `JwtFilter`는 블랙리스트(`TokenBlacklistRepository`)에 오른 토큰을 거부한다(로그아웃 시 무효화). `/auth/**`와 `/api/v1/auth/oauth2/**`는 access 검증을 건너뛴다(`shouldNotFilter`, 재발급·로그아웃은 refresh 쿠키로 동작). 인증 실패(401)는 `RestAuthenticationEntryPoint`, 인가 거부(403)는 `RestAccessDeniedHandler`가 `ErrorResponse`로 응답한다. 쿠키 인증이라 CORS는 `allowCredentials(true)` + `app.cors.allowed-origin-patterns`(와일드카드 `*` 불가, 패턴 목록)로 구성한다.
...
- 성공 응답은 `ApiResponse.ok(data)` / `ApiResponse.ok(message, data)` / `ApiResponse.ok()` 사용, 컨트롤러는 **`ApiResponse<T>`를 직접 반환**한다 — `ResponseEntity`로 감싸지 않는다(`@RestController`라 `RequestResponseBodyMethodProcessor`가 그대로 직렬화하고 상태는 200이 된다)
- **200이 아닌 성공 상태는 `@ResponseStatus`로 선언한다.** `ApiResponse.created(...)`는 `@ResponseStatus(HttpStatus.CREATED)`, `ApiResponse.accepted(...)`는 `@ResponseStatus(HttpStatus.ACCEPTED)`와 반드시 짝을 이룬다 — 애노테이션을 빠뜨리면 바디 `status`는 `"201"`인데 실제 HTTP는 200으로 나가 계약이 깨진다. 바디 `status` 문자열과 HTTP 상태코드를 항상 같은 값으로 유지할 것. `@ResponseStatus`의 `reason` 속성은 **쓰지 않는다** — `sendError()` 경로로 빠져 `ApiResponse` 바디가 통째로 사라지고 컨테이너 에러 페이지가 나간다
- 응답 헤더·쿠키가 필요하면 `HttpServletResponse` 파라미터를 받아 조작한다(인증 쿠키가 이 방식 — `CookieProvider`). `ResponseEntity`는 **조건부 상태코드**(같은 메서드가 실행 결과에 따라 다른 상태를 반환)나 **ETag·조건부 GET(304) 처리**가 필요한 경우에만 쓰고, 사유를 주석으로 남긴다 — 이 두 가지는 `@ResponseStatus`로 대체할 수 없다
...
| 2026-09-14 | **Bearer 헤더 인증 서술 정정(stale 5곳)** — PR #169(`fix/cookie-only-auth`, 커밋 `bdac837`)이 Bearer 헤더 인증을 제거하고 `access_token` 쿠키 단일 경로로 통일했는데, 하네스는 2026-07-20 동기화 당시의 "Bearer 헤더 우선·쿠키 폴백"으로 남아 있었다(그 시점엔 사실이었다). 헤더로 우회가 안 된다는 걸 모르면 서버-투-서버 호출·부하 테스트가 401로 막히고 원인을 못 찾는다(k6 인증 디버깅이 실제로 여기서 시간을 썼다). 같은 줄의 `shouldNotFilter` 경로 누락(`/api/v1/auth/oauth2/**`)도 함께 보강. spring-qa의 인증 테스트 예시는 세 군데가 이 프로젝트에서 동작하지 않아 교체 — `@WithUserDetails`는 `UserDetailsService` 빈이 없어 불가, `@WithMockUser`는 principal이 스프링 `User` 타입이라 `@AuthenticationPrincipal CustomUserDetails`에 null이 꽂히고, `Authorization: Bearer` 헤더는 인증되지 않는다. 실제 방식(`authentication(auth)` RequestPostProcessor로 `CustomUserDetails` 직접 주입, `PendingReviewControllerAuthTest` 선례)과 쿠키 통합 테스트 예시로 대체 | CLAUDE.md, .claude/agents/security-developer.md, .claude/skills/spring-security-impl(SKILL.md·references/jwt-impl.md), .claude/skills/spring-qa/SKILL.md | 인증·인가 구조 설명 요청 중 코드-문서 불일치 발견 |
| 2026-09-10 | **훅 2건 신설 + 관측성/QA/k6 부하테스트 인프라 문서 동기화.** 훅: ① `warn-migration-schema.js`(PostToolUse, 신규) — 신규 Flyway 마이그레이션의 `CREATE TABLE`/`CREATE INDEX`/`RENAME`/`ADD CONSTRAINT UNIQUE`가 `core.` 스키마를 명시하지 않으면 리마인더 주입(차단 아님, `warn-native-query.js`와 같은 패턴). V33 사고(#223)로 CLAUDE.md에 가장 반복적으로 박힌 규칙인데도 자동 체크가 없던 것을 메움. ② `guard-paths.js` 확장 — 신규 마이그레이션 Write 시 로컬에 동일 `Vn` 버전이 이미 존재하면 하드 블록(Flyway 버전 중복 불가). 둘 다 lookahead 백트래킹으로 `CREATE TABLE IF NOT EXISTS core.foo`류가 오탐되지 않는지 실제 stdin 페이로드로 검증 후 반영. 문서 동기화: 지난 하네스 점검(2026-08-16) 이후 develop에 코드만 앞서 있던 세 영역을 따라잡음 — (a) `qa-e2e/`(Playwright API 테스트, Gradle 밖 별도 npm 프로젝트) → spring-qa·qa-reviewer·CLAUDE.md Commands에 반영, (b) `deploy/monitoring/`(Loki·Tempo·Alloy·Grafana·Prometheus, k6 결과는 Datadog으로 분리) + `devtools/K6ScenarioSeedRunner`·`K6AdjusterSeedRunner`(부하테스트 데이터 시더, 프로파일+프로퍼티 이중 게이트) + `scripts/adjuster-loadtest.k6.js` → spring-infra·infra-developer에 반영, (c) `POST /auth/dev/login` 백도어가 dev 환경까지 확장되며 갖춘 3중 방어(prod 하드 블록·dev fail-closed·시크릿 상수시간 비교) → spring-security-impl·security-developer에 반영. | .claude/hooks/warn-migration-schema.js(신규), .claude/hooks/guard-paths.js, .claude/settings.json, .claude/skills/{spring-infra,spring-qa,spring-security-impl}/SKILL.md, .claude/agents/{infra-developer,security-developer,qa-reviewer}.md, CLAUDE.md | "코드 전반을 보고 harness setting에 수정할 것·넣으면 좋을 hook 체크" 요청 → 감사 결과(core 스키마 강제 hook 부재, qa-e2e/관측성/k6/dev-login 확장이 미문서화) 확인 후 "1번·3번 훅 넣고 문서 최신화" 승인 |
| 2026-09-27 | **컨트롤러 성공 응답을 `ResponseEntity<ApiResponse<T>>` → `ApiResponse<T>` 직접 반환으로 전환(규칙 + 컨트롤러 21개 파일 52개 메서드).** 200 응답 45곳은 `ResponseEntity.ok()` 래핑이 하는 일이 없어 걷어냈다 — `HttpEntityMethodProcessor`가 이미 200인 상태를 한 번 더 `setStatus`하고 비어 있는 헤더맵을 복사할 뿐이라, 벗기면 `RequestResponseBodyMethodProcessor`가 같은 바디를 그대로 직렬화한다. 200이 아닌 성공 상태 7곳(201 여섯·202 하나)은 `@ResponseStatus`로 옮겨 기존 HTTP 상태를 유지했다. **애노테이션 누락이 곧 계약 파괴**다 — 바디 `status`는 `"201"`인데 실제 HTTP는 200으로 나간다. 이 과정에서 **`/v3/api-docs`가 51개 엔드포인트를 전부 `200`으로만 문서화하던 드리프트를 발견·해소**했다: springdoc은 메서드 바디의 `ResponseEntity.status(201)`을 읽지 못하고 `@ResponseStatus`만 본다(전환 전후 api-docs를 덤프해 201 여섯·202 하나로 교정되는 것을 실측). `reason` 속성 금지(`sendError` 경로로 빠져 `ApiResponse` 바디가 통째로 소실)와 `ResponseEntity` 잔류 허용 범위(조건부 상태코드·ETag/304 — `@ResponseStatus`로 대체 불가)도 함께 명문화. 쿠키 발급은 `HttpServletResponse` 파라미터로 처리하므로 무관. | CLAUDE.md, domain/*/controller/* 21개 파일 | 200 고정 응답의 `ResponseEntity` 래핑을 걷어내자는 요청 — 영향 조사 중 api-docs 상태코드 드리프트를 함께 발견 |
