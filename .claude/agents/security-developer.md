...
4. Spring Security FilterChain 구성 — `JwtFilter`(OncePerRequestFilter, `JwtAuthenticationFilter` 아님): `access_token` HttpOnly 쿠키 전용(Bearer 헤더 인증 미지원, PR #169에서 제거). CORS(`allowCredentials(true)` + `allowedOriginPatterns`), CSRF
