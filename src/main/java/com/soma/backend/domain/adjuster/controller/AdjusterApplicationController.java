 ...
 * 대상은 항상 principal 본인이며(@CurrentUserId), 별도 역할 게이트 없이 로그인만 요구한다.
 */
@RestController
...
public class AdjusterApplicationController {
  ...
  public ResponseEntity<ApiResponse<CreateAdjusterApplicationResponse>> apply(
    ...
    CreateAdjusterApplicationResponse data = adjusterApplicationCommandService.apply(userId, request);
  ...
  public ResponseEntity<ApiResponse<AdjusterApplicationResponse>> getMyApplication(
      @CurrentUserId UUID userId) {
    AdjusterApplicationResponse data = adjusterApplicationQueryService.getMyApplication(userId);
