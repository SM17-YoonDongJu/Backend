...
5. **전술적 DDD 설계 — Bounded Context 배치, Aggregate 경계·불변식, Value Object, 도메인 이벤트, 레이어(domain/application/presentation/infrastructure) 배치 도출**
...
- **전술적 DDD로 설계한다 — `ddd-tactical` 스킬을 참조한다.** 어떤 Aggregate가 어떤 불변식을 갖는지, 무엇이 Value Object인지, 유스케이스(application service) 단위와 트랜잭션 경계, 컨텍스트 간 협력(포트/도메인 이벤트)을 design.md에 명시한다. 신규 코드는 4계층 구조로, 기존 레이어드 코드 수정은 점진 마이그레이션 여부를 판단해 표기한다
...
- 레이어 배치 (domain/application/presentation/infrastructure) 및 유스케이스별 트랜잭션 경계
- 컨텍스트 간 협력 방식 (포트 인터페이스 / 도메인 이벤트)
### API 계약
### DB 스키마
