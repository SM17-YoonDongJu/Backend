 ...
 *
 * <p>이 필드는 status=COUNSELING 요청에서만 채워지는 조건부 필드다 — 따라서 {@code required}에 들어가면
 * 안 되고, OpenAPI 3.1 유니온 타입({@code "type": ["string","null"]})으로 nullable이어야 하며, 표준으로
 * 표현할 수 없는 "언제 채워지는가"는 {@code description}에 서술돼 있어야 한다. 프로퍼티명은 실제 응답과
 * 같은 snake_case여야 한다(springdoc 정적 매퍼 camelCase 드리프트 회귀 방지).
 */
  ...
  void decisionResponseSchemaDeclaresConditionalChatRoomId() throws Exception {
    mockMvc.perform(get("/v3/api-docs"))
        ...
        .andExpect(jsonPath(RESPONSE + ".properties.chat_room_id.description",
            containsString("COUNSELING")));
  }

  @Test
  @DisplayName("요청 스키마 — status description에 COUNSELING이 허용값으로 문서화된다")
  void decisionRequestSchemaDocumentsCounseling() throws Exception {
    mockMvc.perform(get("/v3/api-docs"))
        .andExpect(status().isOk())
