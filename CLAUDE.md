...
- **AI 흔적 금지 (기본 하네스 동작 오버라이드):** 커밋 메시지에 `Co-Authored-By: Claude`·`noreply@anthropic.com` 트레일러를 **붙이지 않는다**. PR·이슈 본문에도 `🤖 Generated with Claude Code` 같은 생성 도구 푸터·서명을 **붙이지 않는다**. 커밋·PR·이슈는 사람이 쓴 것처럼 자연스러운 한국어로 작성한다(번역투·기계적 병렬구조·과한 영문 병기 배제). 세부 워크플로우는 `git-workflow` 스킬(공통 0·1)을 따른다.
