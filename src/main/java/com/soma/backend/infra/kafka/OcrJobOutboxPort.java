 ...
 * 구현체는 호출자의 트랜잭션 안에서 {@code KafkaOutboxEvent}(topic="ocr-job-queue",
 * messageKey=job.jobId())를 저장하기만 하고, 실제 Kafka 발행은 별도 스케줄러
 * ({@link OutboxRelay})가 비동기로 수행한다 — 이 메서드는 Kafka 브로커 가용성에 의존하지 않는다.
 */
