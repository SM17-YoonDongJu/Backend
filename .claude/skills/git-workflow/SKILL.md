...
## 공통 2: 코드 인용 최소화 (AI 티 방지)

AI가 쓴 커밋·PR은 큰 코드 블럭이나 diff 덤프를 본문에 통째로 붙이는 경향이 있다. 사람은
그러지 않는다 — 변경은 이미 PR "Files changed"에 있으니 본문에서 되풀이하지 않는다.

- 커밋·PR 본문에 **큰 fenced 코드 블럭(```)·before/after diff 덤프를 넣지 않는다.**
- 변경 지점은 인라인 백틱으로 **가리키기만** 한다 — `파일:라인`, `메서드명`, 경로.
- 리뷰에 정말 필요한 경우에만 **핵심 1~3줄**을 짧게 인용한다(전체 블록 나열 금지).
- **커밋 메시지 본문은 코드 블럭 없이** 명사형 서술로만 쓴다.

> 예외: `chore/config`처럼 설정 스니펫 자체가 변경의 요점일 때는 짧은 블록을 허용한다.
> 그래도 "무엇이 왜 바뀌었나" 서술이 먼저고, 블록은 근거로 짧게 곁들인다.
...
- 큰 코드 블럭·diff 덤프를 본문에 넣지 않는다(공통 2). 변경 지점은 인라인 `파일:라인`·경로로 가리키고, 꼭 필요하면 핵심 몇 줄만 짧게 인용한다.
