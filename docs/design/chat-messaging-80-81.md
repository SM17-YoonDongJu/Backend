...
1. **상담 수락 = chat 도메인 소유.** `ChatConsultationCommandService.accept`가 report 엔티티를 직접 호출: 내 `ReportReview.accept()`(→ACCEPTED) + 형제 제안 `reject()`(→REJECTED) + `Report.accept(adjusterId)`(COUNSELING→CLOSED) + SYSTEM 메시지. **형제 방만 CLOSED**(내 방은 ACTIVE 유지 — 수락 후 담당 사정사와 대화 지속). 엔드포인트 `PATCH /chats/{chatRoomId}/accept`.
...
- 동작(단일 트랜잭션): 내 제안(`report_review_id`) `ReportReview.accept()` + 형제 제안(같은 report_id) `reject()` + `Report.accept()`(COUNSELING→CLOSED) + SYSTEM 메시지. **형제 방만 `close()` — 내 방은 ACTIVE 유지**(수락 후에도 담당 사정사와 대화 지속). 형제 방에는 "다른 사정사 선택" 종료 SYSTEM 메시지를 각 방 토픽으로 브로드캐스트.
- data: `{ chat_room_id, chat_room_status:"ACTIVE", review_status:"ACCEPTED", report_id, report_status:"CLOSED" }` (내 방은 유지, report만 종결)
...
- 수락: 소유자 아님 403, 검색 방(report_review_id null) 409, review ACCEPTED·형제 REJECTED·report CLOSED·내 방 ACTIVE 유지·형제 방 CLOSED 원자성.
...
6. 거절은 방 CLOSED, 수락은 내 방 ACTIVE 유지(대화 지속)·형제 방만 CLOSED. 종료된 방만 읽기 전용/숨김(프론트).
```
